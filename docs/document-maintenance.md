...
### 역할 기반 에이전트 운영 연계 (후속)
- 문서/코드/운영 역할별 에이전트 분담은 후속 단계에서 도입을 검토합니다.
- 도입 시 책임 경계와 문서 소유권(`Owner`) 매핑 기준을 함께 정의합니다.
...
- 역할 기반 에이전트 분담 정책 문서 추가 필요(TBD)

## 변경 이력
| 날짜 | 변경 내용 | 작성자 |
| --- | --- | --- |
| 2026-03-09 | 역할 기반 에이전트 운영 연계 검토 항목 추가 | Codex |
