...
- MVP의 업그레이드 상호작용은 `동일 역할 건물 교체` 방식으로 통일합니다.
- 건물 상세 UI의 `제자리 업그레이드`는 Post-MVP 후보로 이관합니다.
...
- 업그레이드 조작 방식은 교체형(예: Miner Mk1 -> Miner Mk2)으로 고정합니다.
- Tier 3 건물은 최종 목표(코어 채굴/탈출 제작)와 직접 연결합니다.

## 미해결 이슈
- 교체 업그레이드 시 기존 건물 환급/철거 비용 규칙 확정(TBD)

## 변경 이력
| 날짜 | 변경 내용 | 작성자 |
| --- | --- | --- |
| 2026-03-10 | 업그레이드 상호작용을 교체형으로 확정하고 제자리 업그레이드는 Post-MVP로 분리 | Codex |
